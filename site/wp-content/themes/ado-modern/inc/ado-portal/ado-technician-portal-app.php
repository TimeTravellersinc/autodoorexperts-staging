...
function ado_tp_render_view(string $view, array $ctx): string
{
    ...
    if ($view === 'schedule') {
        ...
        if (!($project instanceof WC_Order)) {
            ...
            ?>
            <div class="page-header"><div><div class="page-title"><?php echo esc_html($project_name); ?></div><div class="page-sub"><?php echo esc_html($project_location); ?> &middot; Project #<?php echo esc_html((string) $project->get_id()); ?></div></div></div>
            <div class="card" style="margin-bottom:12px;"><div class="card-header"><div class="card-title">Project Overview</div></div><div class="card-body"><div class="kv">Order ID: <?php echo esc_html((string) $project->get_id()); ?></div><div class="kv">Status: <?php echo esc_html(ucfirst((string) $project->get_status())); ?></div><div class="kv">Next Visit: <?php echo esc_html($next_visit !== '' ? $next_visit : 'Not scheduled'); ?></div><div class="kv">Door Count: <?php echo esc_html((string) count((array) $project_doors)); ?></div><div class="kv">Progress: <?php echo esc_html((string) max(0, min(100, $project_progress))); ?>%</div></div></div>
            <div class="card"><div class="card-header"><div class="card-title">Doors</div></div><div class="card-body"><?php if (empty($project_doors)) { ?><div class="ado-empty">No doors are available for this project yet.</div><?php } else { ?><div class="list"><?php foreach ($project_doors as $door) { ?><div class="list-item"><strong><?php echo esc_html((string) ($door['door_id'] ?? 'Door')); ?></strong><small><?php echo esc_html((string) ($door['model'] ?? 'Model pending')); ?></small></div><?php } ?></div><?php } ?></div></div>
